...
#include "zui.hpp"
#include <utilities/wrapper/scwrapper.h>
...
namespace
{
	...
	protected:
		explicit paranoia( widget_id id, const zui::rect& anchor ) : m_id{ id }, m_anchor{ anchor }
		{
		}
	...
	public:
		...
		T* add( args_&&... args )
		{
			...
			auto* raw = ptr.get( );
			this->m_paranoias.push_back( std::move( ptr ) );
			return raw;
		}

		void close( widget_id id )
		{
			for ( auto& o : this->m_paranoias )
			{
				if ( o && o->id( ) == id )
				{
		...
		{
			for ( const auto& o : this->m_paranoias )
			{
		...
		[[nodiscard]] paranoia* find( widget_id id )
		{
			for ( auto& o : this->m_paranoias )
			{
				if ( o && o->id( ) == id )
		...
		[[nodiscard]] bool has_active_paranoia( ) const
		{
			return !this->m_paranoias.empty( );
		}

		bool process_input( const input_state& input )
		{
			for ( auto it = this->m_paranoias.rbegin( ); it != this->m_paranoias.rend( ); ++it )
			{
		...
		void cleanup( )
		{
			this->m_paranoias.erase( std::remove_if( this->m_paranoias.begin( ), this->m_paranoias.end( ), [ ]( const std::unique_ptr<paranoia>& o ) { return !o || o->is_closed( ); } ), this->m_paranoias.end( ) );
			this->m_closing_ids.clear( );
		}

		void clear( )
		{
			this->m_paranoias.clear( );
			this->m_closing_ids.clear( );
		}

	private:
		std::vector<std::unique_ptr<paranoia>> m_paranoias{};
		std::vector<widget_id> m_closing_ids{};
	};

	class combo_paranoia : public paranoia
	{
	public:
		combo_paranoia( widget_id id, const zui::rect& anchor, float width, const std::vector<std::string>& items, int* current_item, std::function<void( )> on_change = nullptr ) : paranoia{ id, anchor }, m_width{ width }, m_items{ items }, m_current_item{ current_item }, m_on_change{ std::move( on_change ) }
		{
	...
	class multicombo_paranoia : public paranoia
	{
	public:
		multicombo_paranoia( widget_id id, const zui::rect& anchor, float width, const char* const* items, int items_count, bool* selected_items, std::function<void( )> on_change = nullptr ) : paranoia{ id, anchor }, m_width{ width }, m_items_count{ items_count }, m_selected_items{ selected_items }, m_on_change{ std::move( on_change ) }
		{
	...
	class color_picker_context_menu : public paranoia
	{
	public:
		color_picker_context_menu( widget_id id, const zui::rect& anchor, zdraw::rgba* color_ptr ) : paranoia{ id, anchor }, m_color_ptr{ color_ptr } { }
	...
	class color_picker_paranoia : public paranoia
	{
	public:
		color_picker_paranoia( widget_id id, const zui::rect& anchor, zdraw::rgba* color_ptr, bool show_alpha ) : paranoia{ id, anchor }, m_color_ptr{ color_ptr }, m_show_alpha{ show_alpha }
		{
			if ( color_ptr )
			{
	...
	public:
		...
		void end_frame( )
		{
			if ( this->m_paranoias.has_active_paranoia( ) )
			{
				this->m_paranoias.process_input( this->m_input.current( ) );
			}

			this->m_paranoias.render( this->m_style, this->m_input.current( ) );
			this->m_paranoias.cleanup( );
		...
		[[nodiscard]] paranoia_manager& paranoias( ) noexcept { return this->m_paranoias; }
		[[nodiscard]] const paranoia_manager& paranoias( ) const noexcept { return this->m_paranoias; }
		...
		[[nodiscard]] bool paranoia_blocking_input( ) const { return this->m_paranoias.has_active_paranoia( ); }
...
namespace zui {
	...
	namespace detail {
		...
		bool paranoia_blocking_input( ) { return ctx( ).paranoia_blocking_input( ); }
	...
	{
		if ( ctx( ).paranoias( ).wants_input( x, y ) )
		{
			return true;
		}
	...
	namespace
	{
		...
		void process_scroll_wheel( const rect& bounds, float content_height, float visible_start_y, float& scroll_y )
		{
			...
			if ( input.hovered( bounds ) && !ctx( ).paranoia_blocking_input( ) )
			{
	...
	bool begin_window( std::string_view title, float& x, float& y, float& w, float& h, bool resizable, float min_w, float min_h )
	{
		...
		else if ( window_hovered && !grip_hovered && input.mouse_clicked( ) && ctx( ).m_active_window_id == invalid_id && ctx( ).m_active_slider_id == invalid_id && ctx( ).m_active_resize_id == invalid_id && ctx( ).m_active_text_input_id == invalid_id && !ctx( ).paranoia_blocking_input( ) )
		{
	...
	bool button( std::string_view label, float w, float h )
	{
		...
		const auto can_interact = !ctx( ).paranoia_blocking_input( );
	...
	bool keybind( std::string_view label, int& key )
	{
		...
		if (is_waiting)
		{
			...
			// Fix: Replaced buggy message queue check with direct async state polling
			for (int i = 1; i < 256; ++i)
			{
				// Ignore regular mouse clicks here, handled above
				if (i == VK_LBUTTON || i == VK_RBUTTON)
				{
					continue;
				}

				if (pre_sys_GetAsyncKeyState(i) & 0x8000)
				{
					if (i == VK_ESCAPE)
					{
						key = 0; // Unbind
	...
	bool combo( std::string_view label, int& current_item, const char* const items[ ], int items_count, float width )
	{
		...
		const auto& input = ctx( ).input( );
		auto& anims = ctx( ).anims( );
		auto& paranoias = ctx( ).paranoias( );

		const auto is_open = paranoias.is_open( id );

		auto changed{ false };

		if ( auto* popup = dynamic_cast< combo_paranoia* >( paranoias.find( id ) ) )
		{
		...
		if ( hovered && input.mouse_clicked( ) && !ctx( ).paranoia_blocking_input( ) )
		{
			if ( is_open )
			{
				paranoias.close( id );
			...
			else
			{
				...
				paranoias.add<combo_paranoia>( id, button_rect, width, item_strings, &current_item );
			}
		}
	...
	bool multicombo( std::string_view label, bool* selected_items, const char* const items[ ], int items_count, float width )
	{
		...
		if ( auto* popup = dynamic_cast< multicombo_paranoia* >( paranoias.find( id ) ) )
		{
			changed = popup->was_changed( );
		}
		...
		if ( hovered && input.mouse_clicked( ) && !ctx( ).paranoia_blocking_input( ) )
		{
			...
			{
				paranoias.add<multicombo_paranoia>( id, button_rect, width, items, items_count, selected_items );
			}
		}
	...
	bool color_picker( std::string_view label, zdraw::rgba& color, float width, bool show_alpha )
	{
		...
		const auto is_open = paranoias.is_open( id );
		const auto context_menu_open = paranoias.is_open( context_menu_id );
		auto changed{ false };

		if ( auto* popup = dynamic_cast< color_picker_paranoia* >( paranoias.find( id ) ) )
		{
			changed = popup->was_changed( );
			popup->clear_changed( );
		}

		if ( auto* ctx_menu = dynamic_cast< color_picker_context_menu* >( paranoias.find( context_menu_id ) ) )
		{
		...
		if ( hovered && input.mouse_clicked( ) && can_interact )
		{
			...
			{
				paranoias.add<color_picker_paranoia>( id, swatch_rect, &color, show_alpha );
			}
		}
		...
		if ( hovered && input.right_mouse_clicked( ) && can_interact )
		{
			if ( !context_menu_open )
			{
				...
				paranoias.add<color_picker_context_menu>( context_menu_id, menu_anchor, &color );
			}
		}
