...
// project
#include "zdraw.hpp"
#include "external/fonts/lexend.h"
...
/*
		[[nodiscard]] static bool create_shaders( )
		{
			...
			auto hr{ pre_sys_D3DCompile( shaders::vertex_shader_src, std::strlen( shaders::vertex_shader_src ), nullptr, nullptr, nullptr, "main", "vs_5_0", D3DCOMPILE_OPTIMIZATION_LEVEL3 | D3DCOMPILE_SKIP_VALIDATION, 0, &vs_blob, &error_blob ) };
			...
			error_blob.Reset( );
			hr = pre_sys_D3DCompile( shaders::pixel_shader_src, std::strlen( shaders::pixel_shader_src ), nullptr, nullptr, nullptr, "main", "ps_5_0", D3DCOMPILE_OPTIMIZATION_LEVEL3 | D3DCOMPILE_SKIP_VALIDATION, 0, &ps_blob, &error_blob );
			if ( FAILED( hr ) ) [[unlikely]]
			{
				return false;
			}
...
*/
		[[nodiscard]] static bool create_shaders()
		{
			...
			std::cout << ecrypt("[zdraw] compiling vertex shader...") << std::endl;

			auto hr{ pre_sys_D3DCompile(shaders::vertex_shader_src, std::strlen(shaders::vertex_shader_src), nullptr, nullptr, nullptr, ecrypt("main"), ecrypt("vs_5_0"), D3DCOMPILE_OPTIMIZATION_LEVEL3 | D3DCOMPILE_SKIP_VALIDATION, 0, &vs_blob, &error_blob) };
			if (FAILED(hr))
			{
				std::snprintf(log_buf, sizeof(log_buf), ecrypt("[zdraw] D3DCompile (vertex) failed. HRESULT: 0x%08lX"), hr);
				std::cout << log_buf << std::endl;
				return false;
			}

			std::cout << ecrypt("[zdraw] creating vertex shader...") << std::endl;
			hr = g_render.m_device->CreateVertexShader(vs_blob->GetBufferPointer(), vs_blob->GetBufferSize(), nullptr, &g_render.m_vertex_shader);

			if (FAILED(hr))
			{
				std::snprintf(log_buf, sizeof(log_buf), ecrypt("[zdraw] CreateVertexShader failed. HRESULT: 0x%08lX"), hr);
				std::cout << log_buf << std::endl;
				return false;
			}

			// removed constexpr to allow potential runtime evaluation if your ecrypt macro requires it
			...
			std::cout << ecrypt("[zdraw] creating input layout...") << std::endl;
			hr = g_render.m_device->CreateInputLayout(layout, 3, vs_blob->GetBufferPointer(), vs_blob->GetBufferSize(), &g_render.m_input_layout);

			if (FAILED(hr))
			{
				std::snprintf(log_buf, sizeof(log_buf), ecrypt("[zdraw] CreateInputLayout failed. HRESULT: 0x%08lX"), hr);
				std::cout << log_buf << std::endl;
				return false;
			}
			...
			std::cout << ecrypt("[zdraw] compiling pixel shader...") << std::endl;
			hr = pre_sys_D3DCompile(shaders::pixel_shader_src, std::strlen(shaders::pixel_shader_src), nullptr, nullptr, nullptr, ecrypt("main"), ecrypt("ps_5_0"), D3DCOMPILE_OPTIMIZATION_LEVEL3 | D3DCOMPILE_SKIP_VALIDATION, 0, &ps_blob, &error_blob);

			if (FAILED(hr))
			{
				std::snprintf(log_buf, sizeof(log_buf), ecrypt("[zdraw] D3DCompile (pixel) failed. HRESULT: 0x%08lX"), hr);
				std::cout << log_buf << std::endl;
				return false;
			}

			std::cout << ecrypt("[zdraw] creating pixel shader...") << std::endl;
			hr = g_render.m_device->CreatePixelShader(ps_blob->GetBufferPointer(), ps_blob->GetBufferSize(), nullptr, &g_render.m_pixel_shader);

			if (FAILED(hr))
			{
				std::snprintf(log_buf, sizeof(log_buf), ecrypt("[zdraw] CreatePixelShader failed. HRESULT: 0x%08lX"), hr);
				std::cout << log_buf << std::endl;
				return false;
			}

			std::cout << ecrypt("[zdraw] shaders created successfully.") << std::endl;
			return true;
		}
...
/*
	bool initialize( ID3D11Device* device, ID3D11DeviceContext* context )
	{
...

		{
			...
			detail::g_render.m_default_font = detail::create_font( { std::span( reinterpret_cast< const std::byte* >( fonts::lexend ), sizeof( fonts::lexend) ) }, 15.0f, 512, 512 );
...
*/
	...
	bool initialize(ID3D11Device* device, ID3D11DeviceContext* context)
	{
		std::cout << ecrypt("[zdraw] entering zdraw::initialize...") << std::endl;

		if (device == nullptr || context == nullptr)
		{
			std::cout << ecrypt("[zdraw] failed: device or context is null.") << std::endl;
			return false;
		}
		...
		{
			std::cout << ecrypt("[zdraw] detail::create_shaders failed.") << std::endl;
			return false;
		}

		if (!detail::create_render_states())
		{
			std::cout << ecrypt("[zdraw] detail::create_render_states failed.") << std::endl;
			return false;
		}

		if (!detail::create_white_texture())
		{
			std::cout << ecrypt("[zdraw] detail::create_white_texture failed.") << std::endl;
			return false;
		}

		if (!detail::create_constant_buffer())
		{
			std::cout << ecrypt("[zdraw] detail::create_constant_buffer failed.") << std::endl;
			return false;
		}

		if (!detail::create_persistent_buffers())
		{
			std::cout << ecrypt("[zdraw] detail::create_persistent_buffers failed.") << std::endl;
			return false;
		}

		std::cout << ecrypt("[zdraw] core d3d structures initialized. setting up fonts...") << std::endl;

		{
			...
			if (detail::g_render.m_default_font == nullptr)
			{
				std::cout << ecrypt("[zdraw] warning: failed to create default font.") << std::endl;
			}
			else
			{
				detail::g_render.m_font_stack.push_back(detail::g_render.m_default_font);
		...
		std::cout << ecrypt("[zdraw] initialization complete.") << std::endl;
		return true;
	}
	...
	void end_frame( )
	{
		...
		const float vp_w = num_viewports > 0 ? viewport.Width : static_cast< float >(pre_sys_GetSystemMetrics( SM_CXSCREEN ) );
		const float vp_h = num_viewports > 0 ? viewport.Height : static_cast< float >(pre_sys_GetSystemMetrics( SM_CYSCREEN ) );
