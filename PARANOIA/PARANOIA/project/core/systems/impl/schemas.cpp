...
namespace systems {
	...
	std::uintptr_t schemas::find_class_binding(const char* class_name)
	{
		...
		{
			const auto schema_system = g::memory.find_vtable_instance(g::modules.schemasystem, ecrypt("CSchemaSystem"));
			if (!schema_system)
			{
				return 0;
			}
			...
			for (std::int32_t i = 0; i < scope_count; ++i)
			{
				...
				if (std::strcmp(scope_name, ecrypt("client.dll")) == 0)
				{
		...
		// Read 'first' element pointer at offset 0x8 (offset 0x0 is 'add_lock')
		const auto first = g::memory.read<std::uintptr_t>(bucket_addr + 8);
		auto element = first;

		while (element)
		{
			// Read the hash key. Note: Game uses 64-bit keys natively, 
			// but reading 32-bit works here to match the 32-bit murmur2 implementation.
			...
			// 'next' pointer is at offset 0x8
			element = g::memory.read<std::uintptr_t>(element + 8);
		}

		// Read 'first_uncommitted' element pointer at offset 0x10 (16)
