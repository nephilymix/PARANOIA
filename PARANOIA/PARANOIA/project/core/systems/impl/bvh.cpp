...
namespace systems {
	...
	void bvh::parse( )
	{
		const auto trace_against_entities_call = g::memory.find_pattern( g::modules.client, ecrypt("E8 ? ? ? ? C7 87 ? ? ? ? ? ? ? ? 48 8D 54 24 ? 48 8B CF") );
		...
		const auto get_surface_data_from_handle_fn = g::memory.find_pattern( g::modules.client, ecrypt("48 63 41 ? 48 8B 0D") );
		...
		const auto hull_vtable = g::memory.find_vtable( g::modules.vphysics2, ecrypt("CRnHullShape") );
		const auto mesh_vtable = g::memory.find_vtable( g::modules.vphysics2, ecrypt("CRnMeshShape") );
