...
namespace systems {
	...
	void collector::collect_players( const std::vector<entities::cached>& raw )
	{
		...
		for ( const auto& entry : raw )
		{
			...
			const auto player_pawn_handle = g::memory.read<std::uint32_t>( entry.ptr + SCHEMA(ecrypt("CCSPlayerController"), "m_hPlayerPawn"_hash ) );
			...
			const auto health = g::memory.read<std::int32_t>( player_pawn + SCHEMA(ecrypt("C_BaseEntity"), "m_iHealth"_hash ) );
			if ( health <= 0 )
			{
				continue;
			}
			...
			p.pawn = player_pawn;
			p.health = health;
			p.team = g::memory.read<std::int32_t>( player_pawn + SCHEMA(ecrypt("C_BaseEntity"), "m_iTeamNum"_hash ) );
			p.invulnerable = g::memory.read<bool>( player_pawn + SCHEMA(ecrypt("C_CSPlayerPawn"), "m_bGunGameImmunity"_hash ) );
			p.armor = g::memory.read<std::int32_t>( player_pawn + SCHEMA(ecrypt("C_CSPlayerPawn"), "m_ArmorValue"_hash ) );
			p.is_scoped = g::memory.read<bool>( player_pawn + SCHEMA(ecrypt("C_CSPlayerPawn"), "m_bIsScoped"_hash ) );
			p.is_defusing = g::memory.read<bool>( player_pawn + SCHEMA(ecrypt("C_CSPlayerPawn"), "m_bIsDefusing"_hash ) );
			p.is_flashed = g::memory.read<float>( player_pawn + SCHEMA(ecrypt("C_CSPlayerPawnBase"), "m_flFlashBangTime"_hash ) ) > 0.0f;
			p.ping = g::memory.read<std::int32_t>( entry.ptr + SCHEMA(ecrypt("CCSPlayerController"), "m_iPing"_hash ) );

			const auto game_scene_node = g::memory.read<std::uintptr_t>( player_pawn + SCHEMA(ecrypt("C_BaseEntity"), "m_pGameSceneNode"_hash ) );
			if ( game_scene_node )
			{
			...
			const auto item_services = g::memory.read<std::uintptr_t>( player_pawn + SCHEMA(ecrypt("C_BasePlayerPawn"), "m_pItemServices"_hash ) );
			if ( item_services )
			{
				p.has_helmet = g::memory.read<bool>( item_services + SCHEMA(ecrypt("CCSPlayer_ItemServices"), "m_bHasHelmet"_hash ) );
				p.has_defuser = g::memory.read<bool>( item_services + SCHEMA(ecrypt("CCSPlayer_ItemServices"), "m_bHasDefuser"_hash ) );
			}

			const auto weapon_services = g::memory.read<std::uintptr_t>( player_pawn + SCHEMA(ecrypt("C_BasePlayerPawn"), "m_pWeaponServices"_hash ) );
			if ( weapon_services )
			{
				const auto active_weapon_handle = g::memory.read<std::uint32_t>( weapon_services + SCHEMA(ecrypt("CPlayer_WeaponServices"), "m_hActiveWeapon"_hash ) );
				if ( active_weapon_handle && active_weapon_handle != 0xFFFFFFFF )
				{
					p.weapon.ptr = systems::g_entities.lookup( active_weapon_handle );
					if ( p.weapon.ptr )
					{
						p.weapon.vdata = g::memory.read<std::uintptr_t>( p.weapon.ptr + SCHEMA(ecrypt("C_BaseEntity"), "m_nSubclassID"_hash ) + 0x8 );
						if ( p.weapon.vdata )
						{
							p.weapon.ammo = g::memory.read<std::int32_t>( p.weapon.ptr + SCHEMA(ecrypt("C_BasePlayerWeapon"), "m_iClip1"_hash ) );
							p.weapon.max_ammo = g::memory.read<std::int32_t>( p.weapon.vdata + SCHEMA(ecrypt("CBasePlayerWeaponVData"), "m_iMaxClip1"_hash ) );

							const auto weapon_name_ptr = g::memory.read<std::uintptr_t>( p.weapon.vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_szName"_hash ) );
							if ( weapon_name_ptr )
							{
			...
			const auto name_ptr = g::memory.read<std::uintptr_t>( entry.ptr + SCHEMA(ecrypt("CCSPlayerController"), "m_sSanitizedPlayerName"_hash ) );
			if ( name_ptr )
			{
			...
			const auto money_services = g::memory.read<std::uintptr_t>( entry.ptr + SCHEMA(ecrypt("CCSPlayerController"), "m_pInGameMoneyServices"_hash ) );
			if ( money_services )
			{
				p.money = g::memory.read<std::int32_t>( money_services + SCHEMA(ecrypt("CCSPlayerController_InGameMoneyServices"), "m_iAccount"_hash ) );
			}
	...
	void collector::collect_items( const std::vector<entities::cached>& raw )
	{
		...
		for ( const auto& entry : raw )
		{
			...
			const auto owner_handle = g::memory.read<std::uint32_t>( entry.ptr + SCHEMA(ecrypt("C_BaseEntity"), "m_hOwnerEntity"_hash ) );
			if ( owner_handle && owner_handle != 0xffffffff )
			{
				continue;
			}

			const auto game_scene_node = g::memory.read<std::uintptr_t>( entry.ptr + SCHEMA(ecrypt("C_BaseEntity"), "m_pGameSceneNode"_hash ) );
			if ( !game_scene_node )
			{
				continue;
			}
			...
			i.subtype = subtype;
			i.origin = g::memory.read<math::vector3>( game_scene_node + SCHEMA(ecrypt("CGameSceneNode"), "m_vecAbsOrigin"_hash ) );
			i.ammo = g::memory.read<std::int32_t>( entry.ptr + SCHEMA(ecrypt("C_BasePlayerWeapon"), "m_iClip1"_hash ) );

			const auto weapon_vdata = g::memory.read<std::uintptr_t>( entry.ptr + SCHEMA(ecrypt("C_BaseEntity"), "m_nSubclassID"_hash ) + 0x8 );
			if ( weapon_vdata )
			{
				i.max_ammo = g::memory.read<std::int32_t>( weapon_vdata + SCHEMA(ecrypt("CBasePlayerWeaponVData"), "m_iMaxClip1"_hash ) );
			}
	...
	void collector::collect_projectiles( const std::vector<entities::cached>& raw )
	{
		...
		for ( const auto& entry : raw )
		{
			...
			{
				const auto fire_count = g::memory.read<int>( entry.ptr + SCHEMA(ecrypt("C_Inferno"), "m_fireCount"_hash ) );
				if ( fire_count <= 0 )
				{
					continue;
				}

				const auto fire_positions_base = entry.ptr + SCHEMA(ecrypt("C_Inferno"), "m_firePositions"_hash );
				const auto fire_active_base = entry.ptr + SCHEMA(ecrypt("C_Inferno"), "m_bFireIsBurning"_hash );
				...
				const auto effect_tick = g::memory.read<std::int32_t>( entry.ptr + SCHEMA(ecrypt("C_Inferno"), "m_nFireEffectTickBegin"_hash ) );
			...
			p.subtype = subtype;
			p.origin = g::memory.read<math::vector3>( game_scene_node + SCHEMA(ecrypt("CGameSceneNode"), "m_vecAbsOrigin"_hash ) );
			p.velocity = g::memory.read<math::vector3>( entry.ptr + SCHEMA(ecrypt("C_BaseEntity"), "m_vecAbsVelocity"_hash ) );
			p.thrower_handle = g::memory.read<std::uint32_t>( entry.ptr + SCHEMA(ecrypt("C_BaseGrenade"), "m_hThrower"_hash ) );
			p.bounces = g::memory.read<std::int32_t>( entry.ptr + SCHEMA(ecrypt("C_BaseCSGrenadeProjectile"), "m_nBounces"_hash ) );

			if ( subtype == projectile_subtype::he_grenade || subtype == projectile_subtype::flashbang )
			{
				const auto detonate_tick = g::memory.read<std::int32_t>( entry.ptr + SCHEMA(ecrypt("C_BaseCSGrenadeProjectile"), "m_nExplodeEffectTickBegin"_hash ) );
				p.detonated = detonate_tick > 0;
			}
			else if ( subtype == projectile_subtype::smoke_grenade )
			{
				p.effect_tick_begin = g::memory.read<std::int32_t>( entry.ptr + SCHEMA(ecrypt("C_SmokeGrenadeProjectile"), "m_nSmokeEffectTickBegin"_hash ) );
				p.smoke_active = g::memory.read<bool>( entry.ptr + SCHEMA(ecrypt("C_SmokeGrenadeProjectile"), "m_bDidSmokeEffect"_hash ) );
			}
			else if ( subtype == projectile_subtype::decoy )
			{
				p.effect_tick_begin = g::memory.read<std::int32_t>( entry.ptr + SCHEMA(ecrypt("C_DecoyProjectile"), "m_nDecoyShotTick"_hash ) );
			}
