...
namespace threads {
	...
	void game( )
	{
		...
		g::console.print("game thread initialized");
	...
	void combat( )
	{
		pre_sys_timeBeginPeriod( 1 );
		g::console.print("combat thread started!");
	...
	{
		std::this_thread::sleep_for(std::chrono::milliseconds(500));
		g::console.print("misc thread initialized");

		while (true)
		{
			if (systems::g_local.valid())
			{
				// update spectators logic here
				features::misc::g_spectators.update();
			}

			// sleep for 100ms since we don't need to update spectators 1000 times a second
