...
namespace settings {
	...
	struct combat
	{
		struct aimbot
		{
			...
			bool rcs{ false };
			float rcs_scale_x{ 2.0f }; // pitch (vertical recoil)
			float rcs_scale_y{ 2.0f }; // yaw (horizontal recoil)
		...
		{
			// Prevent unsigned integer underflow by checking the bounds first
			if (weapon_type < cstypes::pistol)
			{
