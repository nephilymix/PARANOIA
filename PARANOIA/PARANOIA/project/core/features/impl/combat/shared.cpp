...
namespace features::combat {
	...
	void shared::penetration::prepare( std::uintptr_t weapon_vdata, std::uintptr_t weapon )
	{
		...
		{
			.damage = static_cast< float >( g::memory.read<int>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_nDamage"_hash ) ) ),
			.penetration = g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flPenetration"_hash ) ),
			.range_modifier = g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flRangeModifier"_hash ) ),
			.range = g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flRange"_hash ) ),
			.armor_ratio = g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flArmorRatio"_hash ) ),
			.headshot_multiplier = g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flHeadshotMultiplier"_hash ) )
		};
	}
	...
	void shared::tick( )
	{
		...
		const auto weapon_services = g::memory.read<std::uintptr_t>( local_pawn + SCHEMA(ecrypt("C_BasePlayerPawn"), "m_pWeaponServices"_hash ) );
		if ( !weapon_services )
		{
			std::unique_lock lock( this->m_ctx_mutex );
			this->m_ctx = {};
		...
		const auto weapon_handle = g::memory.read<std::uint32_t>( weapon_services + SCHEMA(ecrypt("CPlayer_WeaponServices"), "m_hActiveWeapon"_hash ) );
		if ( !weapon_handle )
		{
			std::unique_lock lock( this->m_ctx_mutex );
			this->m_ctx = {};
		...
		ctx.weapon_vdata = g::memory.read<std::uintptr_t>( ctx.weapon + SCHEMA(ecrypt("C_BaseEntity"), "m_nSubclassID"_hash ) + 0x8 );
		if ( !ctx.weapon_vdata )
		{
			std::unique_lock lock( this->m_ctx_mutex );
			this->m_ctx = {};
		...
		ctx.weapon_type = g::memory.read<std::uint32_t>( ctx.weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_WeaponType"_hash ) );
		ctx.item_def_idx = g::memory.read<std::uint16_t>( ctx.weapon + SCHEMA(ecrypt("C_EconEntity"), "m_AttributeManager"_hash ) + SCHEMA(ecrypt("C_AttributeContainer"), "m_Item"_hash ) + SCHEMA(ecrypt("C_EconItemView"), "m_iItemDefinitionIndex"_hash ) );
		ctx.num_bullets = g::memory.read<int>( ctx.weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_nNumBullets"_hash ) );
		ctx.accuracy_penalty = g::memory.read<float>( ctx.weapon + SCHEMA(ecrypt("C_CSWeaponBase"), "m_fAccuracyPenalty"_hash ) );
		...
		ctx.recoil_index = g::memory.read<float>( ctx.weapon + SCHEMA(ecrypt("C_CSWeaponBase"), "m_flRecoilIndex"_hash ) );
		ctx.is_reloading = g::memory.read<bool>( ctx.weapon + SCHEMA(ecrypt("C_CSWeaponBase"), "m_bInReload"_hash ) );
		...
		ctx.cycle_time = g::memory.read<float>( ctx.weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flCycleTime"_hash ) );
		ctx.last_shot_time = g::memory.read<float>( ctx.weapon + SCHEMA(ecrypt("C_CSWeaponBase"), "m_fLastShotTime"_hash ) );
		ctx.is_full_auto = g::memory.read<bool>( ctx.weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_bIsFullAuto"_hash ) );

		if ( ctx.weapon_type == cstypes::sniper )
		{
			ctx.weapon_ready = ( ctx.current_time - ctx.last_shot_time >= ctx.cycle_time );
			ctx.is_scoped = g::memory.read<bool>( local_pawn + SCHEMA(ecrypt("C_CSPlayerPawn"), "m_bIsScoped"_hash ) );
		}
		else
		{
	...
	math::vector3 shared::extrapolate_stop( const math::vector3& pos ) const
	{
		...
		const auto movement_services = g::memory.read<std::uintptr_t>( pawn + SCHEMA(ecrypt("C_BasePlayerPawn"), "m_pMovementServices"_hash ) );
		if ( !movement_services )
		{
			return pos;
		}

		const auto flags = g::memory.read<std::uint32_t>( pawn + SCHEMA(ecrypt("C_BaseEntity"), "m_fFlags"_hash ) );
		if ( !( flags & 1 ) )
		{
			return pos;
		}

		auto velocity = g::memory.read<math::vector3>( pawn + SCHEMA(ecrypt("C_BaseEntity"), "m_vecAbsVelocity"_hash ) );
		velocity.z = 0.0f;
		...
		const auto surface_friction = g::memory.read<float>( movement_services + SCHEMA(ecrypt("CPlayer_MovementServices_Humanoid"), "m_flSurfaceFriction"_hash ) );
		const auto max_speed = g::memory.read<float>( movement_services + SCHEMA(ecrypt("CPlayer_MovementServices"), "m_flMaxspeed"_hash ) );
		const auto player_friction = g::memory.read<float>( pawn + SCHEMA(ecrypt("C_BaseEntity"), "m_flFriction"_hash ) );
		const auto current_time = this->m_ctx.current_time;
		const auto tick_complete_time = g::memory.read<float>( movement_services + SCHEMA(ecrypt("CCSPlayer_MovementServices"), "m_flOffsetTickCompleteTime"_hash ) );
		const auto stashed_speed = g::memory.read<float>( movement_services + SCHEMA(ecrypt("CCSPlayer_MovementServices"), "m_flOffsetTickStashedSpeed"_hash ) );
		...
		const auto buttons = g::memory.read<std::uintptr_t>( movement_services + SCHEMA(ecrypt("CPlayer_MovementServices"), "m_nButtons"_hash ) );
		const auto ducking_state = g::memory.read<bool>( movement_services + SCHEMA(ecrypt("CCSPlayer_MovementServices"), "m_bDucking"_hash ) );
		...
		const auto water_level = g::memory.read<float>( pawn + SCHEMA(ecrypt("C_BaseEntity"), "m_flWaterLevel"_hash ) );
		...
		{
			const auto wpn_speed = g::memory.read<float>( this->m_ctx.weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flMaxSpeed"_hash ) );
			weapon_speed_ratio = std::fminf( 1.0f, wpn_speed / 250.0f );

			if ( this->m_ctx.weapon )
			{
				const auto zoom = g::memory.read<std::int32_t>( this->m_ctx.weapon + SCHEMA(ecrypt("C_CSWeaponBaseGun"), "m_zoomLevel"_hash ) );
				const auto zoom_count = g::memory.read<std::int32_t>( this->m_ctx.weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_nZoomLevels"_hash ) );
	...
	bool shared::is_sniper_accurate( ) const
	{
		...
		const auto camera_services = g::memory.read<std::uintptr_t>( pawn + SCHEMA(ecrypt("C_BasePlayerPawn"), "m_pCameraServices"_hash ) );
		if ( !camera_services )
		{
			return false;
		}

		auto scope_inaccuracy = g::memory.read<float>( camera_services + SCHEMA(ecrypt("CCSPlayer_CameraServices"), "m_vClientScopeInaccuracy"_hash ) );
	...
	float shared::get_prediction_time( ) const
	{
		...
		const auto ping = g::memory.read<std::int32_t>( controller + SCHEMA(ecrypt("CCSPlayerController"), "m_iPing"_hash ) );
	...
	{
		return g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flSpread"_hash ) );
	}
	...
	float shared::get_inaccuracy( std::uintptr_t pawn, std::uintptr_t weapon, std::uintptr_t weapon_vdata, const math::vector3& eye_angles, float accuracy_penalty ) const
	{
		...
		const auto max_speed = g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flMaxSpeed"_hash ) );
		const auto inaccuracy_move = g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flInaccuracyMove"_hash ) );
		const auto player_velocity = g::memory.read<math::vector3>( pawn + SCHEMA(ecrypt("C_BaseEntity"), "m_vecAbsVelocity"_hash ) );
		const auto speed = player_velocity.length_2d( );
		const auto vertical_speed = std::fabsf( player_velocity.z );
		const auto is_scoped = g::memory.read<bool>( pawn + SCHEMA(ecrypt("C_CSPlayerPawn"), "m_bIsScoped"_hash ) );
		...
		if ( !on_ground )
		{
			const auto inaccuracy_jump_initial = g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flInaccuracyJumpInitial"_hash ) );
			const auto inaccuracy_jump_apex = g::memory.read<float>( weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flInaccuracyJumpApex"_hash ) );
		...
		inaccuracy += g::memory.read<float>( weapon + SCHEMA(ecrypt("C_CSWeaponBase"), "m_flTurningInaccuracy"_hash ) );
