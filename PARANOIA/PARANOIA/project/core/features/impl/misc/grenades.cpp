...
namespace features::misc {
	...
	void grenades::on_render( )
	{
		...
		{
			const auto pin_pulled = g::memory.read<bool>( ctx.weapon + SCHEMA(ecrypt("C_BaseCSGrenade"), "m_bPinPulled"_hash ) );
	...
	bool grenades::can_predict( ) const
	{
		...
		const auto throw_time = g::memory.read<float>( ctx.weapon + SCHEMA(ecrypt("C_BaseCSGrenade"), "m_fThrowTime"_hash ) );
		if ( throw_time > 0.0f )
		{
			return false;
		}
	...
	void grenades::update_weapon_properties( )
	{
		...
		this->m_throw_velocity = std::clamp( g::memory.read<float>( ctx.weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_flThrowVelocity"_hash ) ), 1.0f, 10000.0f );

		const auto name_ptr = g::memory.read<std::uintptr_t>( ctx.weapon_vdata + SCHEMA(ecrypt("CCSWeaponBaseVData"), "m_szName"_hash ) );
		if ( !name_ptr )
		{
	...
	void grenades::setup_throw( math::vector3& origin, math::vector3& velocity )
	{
		...
		if ( pin_pulled )
		{
			strength = std::clamp( g::memory.read<float>( ctx.weapon + SCHEMA(ecrypt("C_BaseCSGrenade"), "m_flThrowStrength"_hash ) ), 0.0f, 1.0f );
	...
	void grenades::update_in_flight( )
	{
		...
		const auto local_pawn_handle = g::memory.read<std::uint32_t>( controller + SCHEMA(ecrypt("CCSPlayerController"), "m_hPlayerPawn"_hash ) );
		const auto now = std::chrono::steady_clock::now( );
		...
		for ( const auto& proj : projectiles )
		{
			...
			if ( existing )
			{
				...
				{
					const auto initial_pos = g::memory.read<math::vector3>( proj.entity + SCHEMA(ecrypt("C_BaseCSGrenadeProjectile"), "m_vInitialPosition"_hash ) );
					const auto initial_vel = g::memory.read<math::vector3>( proj.entity + SCHEMA(ecrypt("C_BaseCSGrenadeProjectile"), "m_vInitialVelocity"_hash ) );
