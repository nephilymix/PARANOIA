...
bool memory::initialize(std::wstring_view process_name)
{
	// Connect to the driver first
	if (!g_driver.Connect())
	{
		return false;
	}
	g::console.print("Driver conected");
	// Find the target process PID
	if (!g_driver.__scrt_a(process_name.data()))
	{
		g::console.print("get PID 1 step Failed!");
		return false;
	}

	this->m_id = g_driver.__scrt_gp();

	if (!this->m_id)
	{
		return false;
		g::console.print("PID 2 step Failed!");
	}

	// Set a dummy handle just in case it is checked elsewhere,
	// since we use driver IOCTLs now, we don't need a real handle.
	HANDLE dHandle = g_driver.get_handle();
	dHandle = reinterpret_cast<HANDLE>(-1);

	// Get base address of the main module
	this->m_base = g_driver.__scrt_gmb(process_name.data());
	g::console.print("main module loaded!");

	if (!g_driver.enable_protection()) {
		g::console.print("Failed to protect app.");
	}
	g::console.print("App protected!");
	// g::console.print( "attached to process." );
...
{
	if (!address || !buffer || !size) [[unlikely]]
	{
		return false;
	}

	// Use driver to read virtual memory
	return g_driver.__scrt_rr(address, buffer, size);
}
...
{
	// Convert narrow string_view to wide string for the driver
	...
	::MultiByteToWideChar(CP_UTF8, 0, name.data(), static_cast<int>(name.size()), &wide_name[0], size_needed);

	// Query module base from driver
	return g_driver.__scrt_gmb(wide_name);
}
...
std::size_t memory::get_module_size(std::uintptr_t module_base) const
{
	...
	// Parse PE headers safely via driver memory reading to get SizeOfImage
	IMAGE_DOS_HEADER dos_header{};
	if (!this->read(module_base, &dos_header, sizeof(dos_header)) || dos_header.e_magic != IMAGE_DOS_SIGNATURE)
	{
		return 0;
	}

	IMAGE_NT_HEADERS nt_headers{};
	if (!this->read(module_base + dos_header.e_lfanew, &nt_headers, sizeof(nt_headers)) || nt_headers.Signature != IMAGE_NT_SIGNATURE)
	{
		return 0;
	}
