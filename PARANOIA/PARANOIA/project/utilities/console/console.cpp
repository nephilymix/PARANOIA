...
bool console::initialize( std::string_view title )
{
	...
	if ( hwnd )
	{
		auto style = ::pre_sys_GetWindowLongW( hwnd, GWL_STYLE );
		style &= ~( WS_MAXIMIZEBOX | WS_SIZEBOX );
		::pre_sys_SetWindowLongW( hwnd, GWL_STYLE, style );

		::pre_sys_SetWindowLongW( hwnd, GWL_EXSTYLE, ::pre_sys_GetWindowLongW( hwnd, GWL_EXSTYLE ) | WS_EX_LAYERED );
		::pre_sys_SetLayeredWindowAttributes( hwnd, 0, 242, LWA_ALPHA );
...
void console::emit( level lvl, const std::string& message ) const
{
	...
	const auto label = detail::gradient_text( ecrypt("paranoia"), grad_from, grad_to );
